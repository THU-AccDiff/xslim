...
import torch
class cache_manager:
    def __init__(self, mode="slow", config_path="cache_utils/xslim_flux_strategy.pth"):
        self.enable_cache = True  
        self.num_steps = 50 
        self.curr_step = 0
        self.num_double_blocks = 19
        self.num_single_blocks = 38
        ...
        self.block_cache_data = {
            "double": {"hidden_states": {}, "encoder_hidden_states": {}},
            "single": {},
        }
    ...
    def _load_config(self, mode, config_path):
        cfg_all = torch.load(config_path, map_location="cpu")
        assert mode in cfg_all, f"Unknown mode: {mode}"
        cfg = cfg_all[mode]
        self.thresh = {k: float(v) for k, v in cfg["thresh"].items()}
    ...
    def set_mode(self, mode):
        self.mode = mode
        self._load_config(mode, self.config_path)
